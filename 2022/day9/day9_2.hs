...
data State = State (Set.Set Pos) [Pos] deriving(Show)

main = do
    handle <- openFile "input" ReadMode
    contents <- hGetContents handle
    let moves = foldl (\s [dir, steps] -> s ++ (replicate (read steps :: Int) (head dir))) "" $ map words $ lines contents
    print $ Set.size $ (\(State set _) -> set) $ foldl processMove (State Set.empty (replicate 10 (Pos 0 0)) ) moves

processMove :: State -> Char -> State
processMove (State visited posList) direction = let posList' = foldl moveKnot [ moveInDirection (head posList) direction ] (tail posList)                                                
                                                    posTail'= last posList'
                                                in State (Set.insert posTail' visited) posList'

moveKnot :: [Pos] -> Pos -> [Pos]
moveKnot listPos currentKnot = listPos ++ [followKnot (last listPos) currentKnot] 
...
followKnot :: Pos -> Pos -> Pos
followKnot (Pos head_x head_y) (Pos tail_x tail_y)
